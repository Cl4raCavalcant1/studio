...
/* --- FIREBASE BRIDGE LOADER (injected) --- */
// Load firebase.js module automatically (non-intrusive).
(function(){
  try {
    // dynamic import if module loading supported
    if (navigator.onLine) {
      import('/firebase.js').then(mod => {
        console.log('firebase.js module loaded');
      }).catch(err => {
        console.warn('failed to import firebase.js:', err);
      });
    } else {
      window.addEventListener('online', function loadOnce(){
        import('/firebase.js').then(mod => {
          console.log('firebase.js module loaded (online)');
        }).catch(err => {
          console.warn('failed to import firebase.js after online:', err);
        });
        window.removeEventListener('online', loadOnce);
      });
    }
  } catch (e) {
    // fallback: try to add script tag as module
...
/* --- SAVE WRAPPER TO TRIGGER FIREBASE SYNC (non-destructive) --- */
(function(){
  try {
    ...
    if (typeof _origSave === 'function') {
      window.save = function(key, value){
        // call original
        _origSave(key, value);
        // try to trigger cloud sync (debounced lightly)
        try {
          if (window.syncSave && typeof window.syncSave === 'function') {
            // small debounce: use a timeout stored on window
            window.__albany_sync_timeout = window.__albany_sync_timeout || null;
            if (window.__albany_sync_timeout) clearTimeout(window.__albany_sync_timeout);
            window.__albany_sync_timeout = setTimeout(function(){
              try {
                window.syncSave({
      ...
      console.log('save() patched to trigger Firebase sync if available.');
    } else {
      console.warn('save() not found; skipping save patch.');
    }
  } catch(e){}
})();
